...
int main (void) {
    ...
    time = (end-begin);
    FILE *tiemposFile;
    tiemposFile = fopen("../tiempos_openmp.txt","a");
...
{
/**
 @brief Calcula (de forma paralela) la media de los gates en la tabla de pulsos tablas y los almacena segun corresponda en el arreglo gates
 @param tablas arreglo de punteros a structs donde se almacenan los valores parseados del archivo pulsos.iq
...
**/
    #pragma omp parallel num_threads(16)
    {
        ...
        int offset = 0;
        float suma = 0;
        #pragma omp for private (muestras,offset,suma) collapse (2)
...
{
/**
 @brief Calcula (de forma paralela) la autocorrelacion de cada pulso
 @param gates arreglo de gates calculado a partir de la media
...
**/
    #pragma omp parallel num_threads(16)
    {
        ...
        float nr1 = 0;
        float nr2 = 0;
        #pragma omp for private (suma,nr1,nr2) collapse (2)
        for (int H_V = 0; H_V < 2; H_V++) {
            for (int nr_gate = 0; nr_gate < 500; nr_gate++) {
                suma = 0;
                #pragma omp parallel for reduction (+:suma)
                for (int pulso = 0; pulso < CANT_PULSOS - 1; pulso++) {
                    nr1 = sqrtf(gates[H_V][pulso][nr_gate][0] * gates[H_V][pulso][nr_gate][0] +
                                gates[H_V][pulso][nr_gate][1] * gates[H_V][pulso][nr_gate][1]);
                    nr2 = sqrtf(gates[H_V][pulso + 1][nr_gate][0] * gates[H_V][pulso + 1][nr_gate][0] +
                                gates[H_V][pulso + 1][nr_gate][1] * gates[H_V][pulso + 1][nr_gate][1]);
                    suma += (nr1 * nr2);
                }
